...
public class MySQLReportDao extends GenericDao<Report> implements ReportDao {
    ...
    @Override
    public List<Report> findUnconfirmedBySpeaker(long speakerId) throws DaoException {
        Connection connection = null;
        try {
            connection = connectionManager.getConnection();
            return findByFields(connection,
                    "SELECT * FROM reports WHERE speaker_id = ? AND status = 'UNCONFIRMED'",
                    speakerId);
        } finally {
            connectionManager.close(connection);
        }
    ...
    public boolean createReportRequest(long reportId, long speakerId) throws DaoException {
        ...
        try {
            connection = connectionManager.getConnection();
            statement = connection.prepareStatement("INSERT  INTO report_request values (?, ?)");
    ...
    public boolean acceptSpeaker(long reportId, long speakerId) throws DaoException {
        ...
        try {
            ...
            statement = connection.prepareStatement("DELETE  FROM report_request WHERE report_id = ? AND  speaker_id = ?");
            statement.setLong(1, reportId);
            statement.setLong(2, speakerId);
    ...
    public boolean rejectReport(long reportId) throws DaoException {
        ...
        try {
            connection = connectionManager.getConnection();
            return updateByField(connection, "UPDATE reports SET speaker_id = NULL, status = 'UNDETAILED' WHERE id = ?", reportId);
        } finally {
          connectionManager.close(connection);
        }
    }
    ...
    @Override
    public boolean deleteALlProposed(long eventId) throws DaoException {
        Connection connection = null;
        try {
            connection = connectionManager.getConnection();
            return deleteByField(connection, "DELETE FROM reports WHERE event_id=? AND status='PROPOSED'", eventId);
        } finally {
            connectionManager.close(connection);
        }
    }
    ...
    @Override
    public boolean updateStatus(long reportId, Report.Status status) throws DaoException {
        Connection connection = null;
        try {
            connection = connectionManager.getConnection();
            return updateByField(connection, "UPDATE reports SET status = ? WHERE id = ?", status.name(), reportId);
        } finally {
            connectionManager.close(connection);
        }
    }
